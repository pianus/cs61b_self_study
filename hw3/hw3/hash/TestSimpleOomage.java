...
public class TestSimpleOomage {
    ...
    public void testHashCodePerfect() {
         ...
         */
        HashSet<Integer> allHashCodes = new HashSet<Integer>();
        int totalHashCodesTillNow = 0;
        for (int r=0; r<=255; r = r +5) {
            for (int g=0; g<=255; g = g +5) {
                for (int b=0; b<=255; b = b +5) {
                    SimpleOomage oomage = new SimpleOomage(r,g,b);
                    allHashCodes.add(oomage.hashCode());
                    totalHashCodesTillNow += 1;
                    assertEquals(allHashCodes.size(), totalHashCodesTillNow);
                }
            }
        }
        assertTrue(true);
